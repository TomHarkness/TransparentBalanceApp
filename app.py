...
def admin_setup():
    ...
    if request.method == 'POST':
        ...
        if action == 'authenticate':
            ...
            if password == ADMIN_PASSWORD:
                ...
                return render_template_string(ADMIN_FORM_TEMPLATE)
            else:
                return render_template_string(ADMIN_AUTH_TEMPLATE, error='Invalid admin password')
        
        # Handle user creation step (requires authentication)
        elif action == 'create_user':
            # Check if user is authenticated
            if not session.get('admin_authenticated'):
                return render_template_string(ADMIN_AUTH_TEMPLATE, error='Authentication required')
            
            # Check session timeout (30 minutes)
            auth_time = session.get('auth_timestamp', 0)
            if time.time() - auth_time > 1800:  # 30 minutes
                session.clear()
                return render_template_string(ADMIN_AUTH_TEMPLATE, error='Session expired. Please authenticate again.')
            ...
            if not user_id:
                return render_template_string(ADMIN_FORM_TEMPLATE, error='Failed to create user')
            
            client_token = generate_client_token(user_id)
            if not client_token:
                return render_template_string(ADMIN_FORM_TEMPLATE, error='Failed to generate CLIENT token')
            
            # Generate consent URL
            consent_url = f"https://consent.basiq.io/home?token={client_token}"
            
            # Clear session after successful user creation
            session.clear()
            
            return render_template_string(ADMIN_SUCCESS_TEMPLATE, 
                                          user_id=user_id,
    ...
    if session.get('admin_authenticated'):
        ...
        if time.time() - auth_time > 1800:  # 30 minutes
            session.clear()
            return render_template_string(ADMIN_AUTH_TEMPLATE, error='Session expired')
        
        # Show business form
        return render_template_string(ADMIN_FORM_TEMPLATE)
    
    # Show authentication form
    return render_template_string(ADMIN_AUTH_TEMPLATE)
...
# Step 1: Authentication only template
ADMIN_AUTH_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <title>Admin Authentication - Basiq Setup</title>
    <style>
        body { 
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; 
            max-width: 500px; margin: 80px auto; padding: 20px; 
        ...
        .error { 
            color: #ef4444; margin: 15px 0; padding: 12px; 
            background: #fef2f2; border-radius: 6px; text-align: left;
        }
        ...
        button { 
            background: #1d1d22; color: white; border: none; 
            padding: 12px 32px; border-radius: 6px; cursor: pointer; 
            font-size: 14px; font-weight: 500; margin-top: 20px;
        }
        button:hover { background: #374151; }
        .security-note { 
            margin-top: 30px; padding: 15px; background: #f9fafb; 
            border-radius: 8px; font-size: 13px; color: #6b7280; text-align: left;
        }
    </style>
</head>
<body>
    <div class="container">
        ...
        <p style="color: #6b7280; margin-bottom: 30px;">
            Enter admin password to access Basiq production setup
        </p>
        ...
        <form method="post">
            ...
            <button type="submit" name="action" value="authenticate">Continue to Setup</button>
        </form>
        
        <div class="security-note">
            <strong>⚠️ Security Notice:</strong><br>
            This is a one-time setup for production CDR compliance. 
            This admin interface should be disabled after successful account connection.
        </div>
    </div>
</body>
</html>
...
# Step 2: Business form template (post-authentication)
ADMIN_FORM_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <title>Business Registration - Basiq Setup</title>
    <style>
        body { 
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; 
        ...
        .session-info {
            background: #e0f2fe; padding: 10px; border-radius: 6px; margin-bottom: 20px;
...
<body>
    <div class="container">
        <div class="session-info">
            ✅ Authenticated - Session expires in 30 minutes
        </div>
        
        <h1>🏢 Business Registration</h1>
        <p>Complete business details for CDR-compliant Basiq account creation.</p>
...
# Step 3: Success template
ADMIN_SUCCESS_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <title>User Created - Consent Required</title>
    <style>
        body { 
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; 
        ...
        .user-id { background: #f0fdf4; padding: 15px; margin: 20px 0; border-radius: 8px; }
        .consent-url { 
            background: #f0f0f0; padding: 20px; margin: 20px 0; 
            border-radius: 8px; word-break: break-all; text-align: left;
        }
        .steps { text-align: left; margin: 20px 0; }
        .warning { background: #fef3cd; padding: 15px; border-radius: 8px; color: #856404; margin-top: 20px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="success">
            ✅ Basiq user created with complete business information
        </div>
        ...
        <div class="consent-url">
            <strong>🔗 Basiq Consent URL:</strong><br>
            <a href="{{ consent_url }}" target="_blank" style="color: #3b82f6; text-decoration: none;">
                {{ consent_url }}
            </a>
        </div>
        ...
        <div class="warning">
            ...
            After completing consent, disable this admin route for security!
        </div>
        
        <p style="margin-top: 30px;">
            <a href="/" style="color: #6b7280;">← Return to Dashboard</a>
        </p>
    </div>
</body>
</html>
